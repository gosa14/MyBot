...
Func getDLLOutput()
	...
	Return $result[0]
EndFunc   ;==>getDLLOutput

Func getNumberOfDrills($listPixelByLevel)
	Local $result = 0

	If IsArray($listPixelByLevel) Then $result = $listPixelByLevel[0]

	SetLog("Total No. of Dark Elixir Drills = " & $result, $COLOR_FUCHSIA)
	Return $result
EndFunc   ;==>getNumberOfDrills

Func fillDrillArray($listPixelByLevel)
	...
	If $numDrills > 0 Then
		For $i = 0 To $numDrills - 1
			...
			If isInsideDiamond($pixel) Then
				...
				; SetLog("Dark Elixir Drill: [" & $result[$i][0] & "," & $result[$i][1] & "], Level: " & $result[$i][2] & ", Hold: " & $result[$i][3] & ", Steal: " & $result[$i][4], $COLOR_BLUE)
			Else
				...
				; SetLog("Found Dark Elixir Drill with an invalid location.", $COLOR_RED)
			EndIf
		Next
	EndIf
...
Func getDrillArray()
	...
	$result = getDLLOutput()
	If $debugSetLog = 1 Then Setlog("Drill search $result[0] = " & $result, $COLOR_PURPLE)
...
Func drillSearch()
	...
	Local $result = getDrillArray()
	Local $listPixelByLevel = StringSplit($result, "~")
