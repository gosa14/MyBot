...
Func getDarkElixir(ByRef $oldSearchDark)
	Local $searchDark = "", $iCount
...
Func smartZap()
	...
	Local Static $oldSearchDark = 0

	Local $strikeOffsets = [7, 10]
	Local $drillLvlOffset, $spellAdjust, $searchDark, $aDarkDrills, $numSpells, $testX, $testY, $tempTestX, $tempTestY, $strikeGain, $expectedDE
	Local $error = 5 ; 5 pixel error margin for de drill search

	; Get the number of lightning spells
	$numSpells = spellCount($eLSpell)
	If $numSpells = 0 Then
	...
	$searchDark = getDarkElixir($oldSearchDark)
	...
	$aDarkDrills = drillSearch()
	; Sort by remaining DE
	_ArraySort($aDarkDrills, 1, 0, 0, 3)
	SetLog("Estimated stealable DE in Drills: " & $aDarkDrills[0][3] & ", " & $aDarkDrills[1][3] & ", " & $aDarkDrills[2][3] & ", " & $aDarkDrills[3][3], $COLOR_FUCHSIA)
	...
	; SetLog("Spell Adjust is: " & $spellAdjust, $COLOR_PURPLE)

	; Loop while you still have spells and the first drill in the array has Dark Elixir, if you are town hall 5 or higher
	While $numSpells > 0 And $aDarkDrills[0][3] <> -1 And $spellAdjust <> -1
		...
		ElseIf $aDarkDrills[0][2] > (4 - $drillLvlOffset) And ($aDarkDrills[0][3] / $DrillLevelHold[$aDarkDrills[0][2] - 1]) > 0.3 Then
			...
			If _Sleep(3500) Then Return
		Else
			SetLog("Not a suitable drill. Removing current drill from list.", $COLOR_FUCHSIA)
			For $i = 0 To UBound($aDarkDrills, 2) - 1
				$aDarkDrills[0][$i] = -1
			Next
		EndIf

		; Store the old DE value and retrieve the new DE value.
		$oldSearchDark = $searchDark
		$searchDark = getDarkElixir($oldSearchDark)
		...
		_ArraySort($aDarkDrills, 1, 0, 0, 3)
		SetLog("Estimated stealable DE in Drills: " & $aDarkDrills[0][4] & ", " & $aDarkDrills[1][4] & ", " & $aDarkDrills[2][4] & ", " & $aDarkDrills[3][4], $COLOR_FUCHSIA)
		...
		If $aDarkDrills[0][0] <> -1 Then
			...
			For $i = 0 To Ubound($aDrills) - 1
				If $aDrills[$i][0] <> -1 Then
					...
					; SetLog("tempX: " & $tempTestX & " tempY: " & $tempTestY, $COLOR_PURPLE)
				EndIf
			Next
			...
			If ($testX > $error Or $testY > $error) And ($testX <> -1 Or $testY <> -1) Then
				For $i = 0 To UBound($aDarkDrills, 2) - 1
					$aDarkDrills[0][$i] = -1
				Next
				SetLog("Removing drill since it wasn't found.", $COLOR_FUCHSIA)
			EndIf
		EndIf
	WEnd
EndFunc   ;==>smartZap
