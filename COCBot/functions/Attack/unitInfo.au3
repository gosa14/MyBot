...
Func spellCount($kind) ; Gets a count of the number of spells of the type specified.
	...
	If $barLocationCCSpell <> -1 And getCCSpellType() = $kind Then
		...
		$numSpells += $numCCSpells
		If $debugSetlog = 1 Then SetLog(($numSpells - $numCCSpells) & " " & $spellText & "s in slot " & $barLocation & " and " & $numCCSpells & " " & $spellText & " in the Clan Castle", $COLOR_PURPLE)
	Else
